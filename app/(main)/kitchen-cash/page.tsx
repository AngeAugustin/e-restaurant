...
import { buildCashSessionName, canReopenKitchenSessionByDate } from "@/lib/cash-session";
...
async function fetchCashSessions(): Promise<CashSession[]> {
  const res = await fetch("/api/kitchen-cash-sessions");
...
export default function CashPage() {
  ...
  const { data: sessions, isLoading } = useQuery({
    queryKey: ["kitchen-cash-sessions"],
  ...
  const canReopenSession = (s: CashSession) =>
    s.status === "CLOSED" &&
    !!latestSession &&
    s._id === latestSession._id &&
    canReopenKitchenSessionByDate(new Date(s.sessionDate));
  ...
  const createNamePreview = buildCashSessionName(new Date(), "KITCHEN");

  const reloadSessions = () => qc.invalidateQueries({ queryKey: ["kitchen-cash-sessions"] });
  ...
  async function updateSessionOpeningFloat(sessionId: string) {
    ...
    setSaving(true);
    const res = await fetch(`/api/kitchen-cash-sessions/${sessionId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ openingFloat }),
    });
  ...
  async function deleteSession(sessionId: string) {
    setSaving(true);
    const res = await fetch(`/api/kitchen-cash-sessions/${sessionId}`, { method: "DELETE" });
    setSaving(false);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
  ...
  async function exportSessionAnalytics(sessionId: string) {
    ...
    const res = await fetch(`/api/kitchen-cash-sessions/${sessionId}/analytics`);
    if (!res.ok) {
  ...
  return (
    <div>
      <PageHeader
        title="Caisse cuisine"
        subtitle="Sessions de caisse dédiées au circuit cuisine"
        action={
          <Button
            onClick={() => {
              setOpenCreate(true);
      ...
      <Dialog open={openCreate} onOpenChange={setOpenCreate}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Ouvrir une session de caisse cuisine</DialogTitle>
            <DialogDescription>
              Le nom est attribué automatiquement. Indépendante de la caisse bar.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!pendingReopen} onOpenChange={(v) => !v && setPendingReopen(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Relance possible jusqu&apos;au lendemain soir de la date de session, s&apos;il n&apos;y a pas d&apos;autre
              session cuisine ouverte.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2">
        ...
        <DialogContent className="max-w-md">
          ...
          (pendingDelete?.financialSummary?.pendingCount ?? 0) > 0 ? (
            <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-950">
              ...
              <div className="space-y-1.5">
                <p className="font-semibold">Des commandes sont liées à cette session</p>
                <ul className="list-disc space-y-0.5 pl-4 text-[13px] leading-relaxed">
                  {(pendingDelete?.financialSummary?.completedCount ?? 0) > 0 ? (
                    <li>
                      {(pendingDelete?.financialSummary?.completedCount ?? 0) > 1
                        ? `${pendingDelete?.financialSummary?.completedCount} commandes clôturées`
                        : "1 commande clôturée"}{" "}
                  ...
                  {(pendingDelete?.financialSummary?.pendingCount ?? 0) > 0 ? (
                    <li>
                      {(pendingDelete?.financialSummary?.pendingCount ?? 0) > 1
                        ? `${pendingDelete?.financialSummary?.pendingCount} commandes encore en attente`
                        : "1 commande encore en attente"}
                    </li>
                  ) : null}
                </ul>
                <p className="text-[13px] leading-relaxed text-amber-900/90">
                  Les commandes ne seront pas supprimées : elles resteront dans Cuisine. En revanche, elles ne seront
                  plus rattachées à une caisse et leurs montants ne seront plus comptés ici.
                </p>
              </div>
            </div>
          ) : null}
